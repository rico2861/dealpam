...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;">
  ...
  <tr><td style="padding:24px 0 8px;text-align:center;">
    ...
    btn(text, url, color = BRAND.orange, textColor = '#fff') {
        return `
    ...
    async sendSellerApproved(to, firstName, storeName) {
        const body = `
    ...
    async sendSellerRejected(to, firstName, reason) {
        const body = `
    ...
    async sendAdminPasswordReset(to, firstName, tempPassword) {
        const body = `
    ...
    async sendAccountLocked(to, firstName, resetUrl) {
        const body = `
    ...
    async sendNewOrderToSeller(to, sellerName, order) {
    ...
    async sendOrderConfirmationToCustomer(to, customerName, order) {
        const itemsHtml = order.items.map(i => `
      <tr>
      ...
      ${this.table([
            ...
            ...(order.sellerPhone ? [['Téléphone', order.sellerPhone]] : []),
    ...
    async sendOrderStatusUpdate(to, customerName, orderNumber, status, detail) {
        const statusMap = {
    ...
    async sendNewMessageNotification(to, recipientName, senderName, preview, chatUrl) {
        const body = `
    ...
    async sendSubscriptionExpiryReminder(to, firstName, planName, daysLeft, renewUrl) {
        const isUrgent = daysLeft <= 3;
        const body = `
      ${this.hero(isUrgent ? '⚠️' : '🔔', isUrgent ? '#FFF1F2' : '#FFFBEB', isUrgent ? 'Abonnement sur le point d\'expirer !' : `Renouvellement dans ${daysLeft} jours`, `Plan ${this.esc(planName)}`)}
      ${this.greeting(firstName)}
    ...
    async sendSubscriptionExpired(to, firstName, planName, renewUrl) {
        const body = `
    ...
    async sendSubscriptionRenewed(to, firstName, planName, endDate) {
    ...
    async sendRaw(to, subject, bodyHtml) {
