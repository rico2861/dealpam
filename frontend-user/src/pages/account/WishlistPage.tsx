...
export default function WishlistPage() {
  ...
  if (!user) return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#060B14', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 2, px: 3, textAlign: 'center' }}>
      ...
      <Typography sx={{ fontWeight: 800, fontSize: 18, color: 'white' }}>Connectez-vous pour voir vos favoris</Typography>
      ...
      <Box sx={{ maxWidth: 1100, mx: 'auto' }}>
        ...
        <Box sx={{ mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
            <Typography sx={{ fontWeight: 900, fontSize: { xs: 20, sm: 26 }, color: 'white', letterSpacing: '-0.5px' }}>
              Mes favoris
            </Typography>
            {!isLoading && (
          ...
          <Typography sx={{ fontSize: 13.5, color: 'rgba(255,255,255,0.3)' }}>
        ...
        {categories.length > 0 && (
          <Box sx={{ display: 'flex', gap: 1, mb: 3, flexWrap: 'wrap', alignItems: 'center' }}>
            <FilterListOutlined sx={{ fontSize: 17, color: 'rgba(255,255,255,0.25)', mr: 0.5 }} />
            {[['all', `Tout (${items.length})`], ...categories.map(([slug, name]) => [slug, `${name} (${items.filter(i => i.product?.category?.slug === slug).length})`])].map(([key, label]) => (
              <Box key={key} onClick={() => setCatFilter(key)} sx={{
                ...
                bgcolor: catFilter === key ? ORANGE : 'rgba(255,255,255,0.04)',
                color: catFilter === key ? 'white' : 'rgba(255,255,255,0.4)',
                border: catFilter === key ? `1.5px solid ${ORANGE}` : '1.5px solid rgba(255,255,255,0.07)',
                transition: 'all 0.18s',
                '&:hover': catFilter === key ? {} : { bgcolor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)' },
              }}>
                {label}
              </Box>
            ))}
        ...
        {isLoading && (
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,1fr)', sm: 'repeat(3,1fr)', md: 'repeat(4,1fr)' }, gap: { xs: 1.5, sm: 2 } }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <Skeleton key={i} variant="rounded" height={300} sx={{ borderRadius: '18px', bgcolor: 'rgba(255,255,255,0.06)' }} />
            ))}
          </Box>
        )}
        ...
        {!isLoading && filtered.length === 0 && (
          <Box sx={{
            ...
            <Typography sx={{ fontWeight: 800, fontSize: 18, color: 'white', mb: 1 }}>
              {catFilter === 'all' ? 'Aucun favori pour l\'instant' : 'Aucun article dans cette catégorie'}
            </Typography>
            <Typography sx={{ fontSize: 13.5, color: 'rgba(255,255,255,0.3)', mb: 3.5 }}>
                  ...
                  <Box component={Link} to={`/products/${p.slug}`} sx={{ display: 'block', position: 'relative', textDecoration: 'none' }}>
                    <Box sx={{
                      ...
                      {img
                        ...
                        : <ShoppingCartOutlined sx={{ fontSize: 36, color: 'rgba(255,255,255,0.1)' }} />
                      }
                    </Box>
                    {/* Discount badge */}
                    {hasDiscount && (
                  ...
                  <Box sx={{ p: { xs: 1.5, sm: 2 }, flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    {p.store?.name && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.6 }}>
                        <StorefrontOutlined sx={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }} />
                        <Typography noWrap sx={{ fontSize: 11, color: 'rgba(255,255,255,0.28)', fontWeight: 600 }}>{p.store.name}</Typography>
                      </Box>
                    )}

                    {/* Name */}
                    <Typography sx={{
                      fontSize: { xs: 12.5, sm: 13.5 }, fontWeight: 700, color: 'rgba(255,255,255,0.82)',
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.8, mb: 1.5 }}>
                      ...
                      {hasDiscount && (
                        <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.28)', textDecoration: 'line-through' }}>
                          {fmt(p.price)}
                        </Typography>
                      )}
                    </Box>
